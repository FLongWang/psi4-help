...
def print_top3_keys(file, top_keys, top2_keys, des=False):
    ...
    if not des:
        for i, keyword in enumerate(top3_element):
            l1="%20s"%keyword
            print(f"{Colors.WHITE}{l1}{Colors.RESET}",end='\t')
            if i % 5 == 4:
                print()
        print()
    else:
...
class Psi4APICLI(cmd.Cmd):
    intro = "\nWelcome to the Psi4 api CLI. Type help or ? to list commands.\n Type \"help\" .\n"
    prompt = "(psi4-api-cli) $ "
    def __init__(self):
        super().__init__()
        file_path = resource_filename('psi4_help', 'psi4_api_class.yaml')
